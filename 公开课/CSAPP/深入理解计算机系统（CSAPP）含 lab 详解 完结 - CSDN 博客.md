...
- [[#深入理解计算机系统|深入理解计算机系统]]
	- [[#1 文章目录^toc|1 文章目录]]
	- [[#2 深入理解计算机操作系统 - 第一章|2 深入理解计算机操作系统 - 第一章]]
		- [[#2.1 信息就是位 + 上下文|2.1 信息就是位 + 上下文]]
		- [[#2.2 程序被其他程序翻译成不同的格式|2.2 程序被其他程序翻译成不同的格式]]
		- [[#2.3 了解编译系统如何工作是大有益处的|2.3 了解编译系统如何工作是大有益处的]]
		- [[#2.4 处理器读并解释储存在内存中的指令|2.4 处理器读并解释储存在内存中的指令]]
		- [[#2.5 系统的硬件组成|2.5 系统的硬件组成]]
			- [[#2.5.1 I/O 设备|2.5.1 I/O 设备]]
			- [[#2.5.2 运行 hello 程序|2.5.2 运行 hello 程序]]
		- [[#2.6 高速缓存至关重要|2.6 高速缓存至关重要]]
		- [[#2.7 存储设备形成层次结构|2.7 存储设备形成层次结构]]
		- [[#2.8 操作系统管理硬件|2.8 操作系统管理硬件]]
			- [[#2.8.1 进程|2.8.1 进程]]
			- [[#2.8.2 线程|2.8.2 线程]]
			- [[#2.8.3 虚拟内存|2.8.3 虚拟内存]]
			- [[#2.8.4 文件|2.8.4 文件]]
		- [[#2.9 系统之间利用网络通信|2.9 系统之间利用网络通信]]
		- [[#2.10 重要主题|2.10 重要主题]]
			- [[#2.10.1 Amdahl 定律|2.10.1 Amdahl 定律]]
			- [[#2.10.2 并发和并行|2.10.2 并发和并行]]
				- [[#2.10.2.1 线程级并发|2.10.2.1 线程级并发]]
				- [[#2.10.2.2 指令级并行|2.10.2.2 指令级并行]]
				- [[#2.10.2.3 单指令、多数据并行|2.10.2.3 单指令、多数据并行]]
			- [[#2.10.3 计算机系统中抽象的重要性|2.10.3 计算机系统中抽象的重要性]]
			- [[#2.10.4 小结|2.10.4 小结]]
	- [[#3 第 2 章：信息的表示和处理|3 第 2 章：信息的表示和处理]]
		- [[#3.1 第一部分：程序结构和执行|3.1 第一部分：程序结构和执行]]
		- [[#3.2 信息存储|3.2 信息存储]]
			- [[#3.2.1 十六进制表示法|3.2.1 十六进制表示法]]
			- [[#3.2.2 字数据大小|3.2.2 字数据大小]]
			- [[#3.2.3 寻址和字节顺序|3.2.3 寻址和字节顺序]]
			- [[#3.2.4 表示字符串|3.2.4 表示字符串]]
			- [[#3.2.5 表示代码|3.2.5 表示代码]]
			- [[#3.2.6 布尔代数简介|3.2.6 布尔代数简介]]
			- [[#3.2.7 C 语言的位级运算|3.2.7 C 语言的位级运算]]
			- [[#3.2.8 C 语言的逻辑运算|3.2.8 C 语言的逻辑运算]]
			- [[#3.2.9 C 语言的移位运算|3.2.9 C 语言的移位运算]]
		- [[#3.3 整型数据表示|3.3 整型数据表示]]
		- [[#3.4 .5 C 语言中的有符号数与无符号数|3.4 .5 C 语言中的有符号数与无符号数]]
		- [[#3.5 关于有符号数，与无符号数|3.5 关于有符号数，与无符号数]]
		- [[#3.6 整数运算|3.6 整数运算]]
			- [[#3.6.1 无符号加法|3.6.1 无符号加法]]
			- [[#3.6.2 补码加法|3.6.2 补码加法]]
			- [[#3.6.3 无符号乘法|3.6.3 无符号乘法]]
			- [[#3.6.4 补码乘法|3.6.4 补码乘法]]
		- [[#3.7 浮点数|3.7 浮点数]]
		- [[#3.8 小结|3.8 小结]]
	- [[#4 第 3 章：程序的机器级表示|4 第 3 章：程序的机器级表示]]
		- [[#4.1 程序编码|4.1 程序编码]]
			- [[#4.1.1 机器级代码|4.1.1 机器级代码]]
			- [[#4.1.2 代码示例|4.1.2 代码示例]]
			- [[#4.1.3 关于格式的注释|4.1.3 关于格式的注释]]
				- [[#4.1.3.1 网络旁注|4.1.3.1 网络旁注]]
			- [[#4.1.4 数据格式|4.1.4 数据格式]]
			- [[#4.1.5 访问信息|4.1.5 访问信息]]
				- [[#4.1.5.1 操作数指示符|4.1.5.1 操作数指示符]]
				- [[#4.1.5.2 数据传送指令|4.1.5.2 数据传送指令]]
				- [[#4.1.5.3 数据传送示例|4.1.5.3 数据传送示例]]
				- [[#4.1.5.4 压入和弹出栈数据|4.1.5.4 压入和弹出栈数据]]
			- [[#4.1.6 算术和逻辑操作|4.1.6 算术和逻辑操作]]
				- [[#4.1.6.1 加载有效地址|4.1.6.1 加载有效地址]]
				- [[#4.1.6.2 一元和二元操作|4.1.6.2 一元和二元操作]]
				- [[#4.1.6.3 移位操作|4.1.6.3 移位操作]]
				- [[#4.1.6.4 特殊的算数操作|4.1.6.4 特殊的算数操作]]
			- [[#4.1.7 控制|4.1.7 控制]]
				- [[#4.1.7.1 条件码|4.1.7.1 条件码]]
				- [[#4.1.7.2 访问条件码|4.1.7.2 访问条件码]]
				- [[#4.1.7.3 跳转指令|4.1.7.3 跳转指令]]
				- [[#4.1.7.4 跳转指令的编码|4.1.7.4 跳转指令的编码]]
				- [[#4.1.7.5 用条件控制来实现条件分支|4.1.7.5 用条件控制来实现条件分支]]
				- [[#4.1.7.6 使用条件传送来实现条件分支|4.1.7.6 使用条件传送来实现条件分支]]
				- [[#4.1.7.7 循环|4.1.7.7 循环]]
					- [[#4.1.7.7.1 dowhile|4.1.7.7.1 dowhile]]
					- [[#4.1.7.7.2 while|4.1.7.7.2 while]]
					- [[#4.1.7.7.3 for|4.1.7.7.3 for]]
				- [[#4.1.7.8 switch 语句|4.1.7.8 switch 语句]]
				- [[#4.1.7.9 过程|4.1.7.9 过程]]
					- [[#4.1.7.9.1 运行时栈|4.1.7.9.1 运行时栈]]
					- [[#4.1.7.9.2 转移控制|4.1.7.9.2 转移控制]]
					- [[#4.1.7.9.3 数据传送|4.1.7.9.3 数据传送]]
					- [[#4.1.7.9.4 栈上的局部存储|4.1.7.9.4 栈上的局部存储]]
					- [[#4.1.7.9.5 寄存器中的局部存储空间|4.1.7.9.5 寄存器中的局部存储空间]]
		- [[#4.2 数组分配和访问|4.2 数组分配和访问]]
					- [[#4.2.1.1.1 基本原则|4.2.1.1.1 基本原则]]
					- [[#4.2.1.1.2 指针运算|4.2.1.1.2 指针运算]]
					- [[#4.2.1.1.3 嵌套的数组|4.2.1.1.3 嵌套的数组]]
					- [[#4.2.1.1.4 定长数组|4.2.1.1.4 定长数组]]
					- [[#4.2.1.1.5 变长数组|4.2.1.1.5 变长数组]]
		- [[#4.3 异质的数据结构|4.3 异质的数据结构]]
					- [[#4.3.1.1.1 联合|4.3.1.1.1 联合]]
					- [[#4.3.1.1.2 数据对齐|4.3.1.1.2 数据对齐]]
				- [[#4.3.1.2 强制对齐|4.3.1.2 强制对齐]]
		- [[#4.4 在机器级程序中将控制与数据结合起来|4.4 在机器级程序中将控制与数据结合起来]]
			- [[#4.4.1 理解指针|4.4.1 理解指针]]
			- [[#4.4.2 应用：使用 GDB 调试器|4.4.2 应用：使用 GDB 调试器]]
			- [[#4.4.3 内存越界引用和缓冲区溢出|4.4.3 内存越界引用和缓冲区溢出]]
			- [[#4.4.4 对抗缓冲区溢出|4.4.4 对抗缓冲区溢出]]
				- [[#4.4.4.1 栈随机化|4.4.4.1 栈随机化]]
				- [[#4.4.4.2 栈破坏检测|4.4.4.2 栈破坏检测]]
				- [[#4.4.4.3 限制可执行代码区域|4.4.4.3 限制可执行代码区域]]
			- [[#4.4.5 支持变长栈帧|4.4.5 支持变长栈帧]]
		- [[#4.5 浮点代码|4.5 浮点代码]]
				- [[#4.5.1.1 浮点传送和转换操作|4.5.1.1 浮点传送和转换操作]]
				- [[#4.5.1.2 过程中的浮点数|4.5.1.2 过程中的浮点数]]
				- [[#4.5.1.3 浮点运算|4.5.1.3 浮点运算]]
			- [[#4.5.2 对浮点代码的观察结论|4.5.2 对浮点代码的观察结论]]
		- [[#4.6 小结|4.6 小结]]
		- [[#4.7 第六章 存储器层次结构|4.7 第六章 存储器层次结构]]
			- [[#4.7.1 存储技术|4.7.1 存储技术]]
				- [[#4.7.1.1 随机访问存储器|4.7.1.1 随机访问存储器]]
					- [[#4.7.1.1.1 静态 RAM|4.7.1.1.1 静态 RAM]]
					- [[#4.7.1.1.2 动态 RAM|4.7.1.1.2 动态 RAM]]
					- [[#4.7.1.1.3 传统的 DRAM|4.7.1.1.3 传统的 DRAM]]
					- [[#4.7.1.1.4 内存模块|4.7.1.1.4 内存模块]]
					- [[#4.7.1.1.5 增强的 DRAM|4.7.1.1.5 增强的 DRAM]]
					- [[#4.7.1.1.6 非易失性存储器|4.7.1.1.6 非易失性存储器]]
					- [[#4.7.1.1.7 访问主存|4.7.1.1.7 访问主存]]
				- [[#4.7.1.2 磁盘存储|4.7.1.2 磁盘存储]]
					- [[#4.7.1.2.1 磁盘构造|4.7.1.2.1 磁盘构造]]
					- [[#4.7.1.2.2 磁盘容量|4.7.1.2.2 磁盘容量]]
					- [[#4.7.1.2.3 磁盘操作|4.7.1.2.3 磁盘操作]]
					- [[#4.7.1.2.4 逻辑磁盘块|4.7.1.2.4 逻辑磁盘块]]
					- [[#4.7.1.2.5 连接 IO 设备|4.7.1.2.5 连接 IO 设备]]
					- [[#4.7.1.2.6 访问磁盘|4.7.1.2.6 访问磁盘]]
				- [[#4.7.1.3 固态硬盘|4.7.1.3 固态硬盘]]
				- [[#4.7.1.4 存储器技术趋势|4.7.1.4 存储器技术趋势]]
			- [[#4.7.2 局部性|4.7.2 局部性]]
				- [[#4.7.2.1 对程序数据引用的局部性|4.7.2.1 对程序数据引用的局部性]]
				- [[#4.7.2.2 取指令的局部性|4.7.2.2 取指令的局部性]]
				- [[#4.7.2.3 局部小结|4.7.2.3 局部小结]]
			- [[#4.7.3 存储器层次结构|4.7.3 存储器层次结构]]
				- [[#4.7.3.1 存储器层次结构中的缓存|4.7.3.1 存储器层次结构中的缓存]]
					- [[#4.7.3.1.1 缓存命中|4.7.3.1.1 缓存命中]]
					- [[#4.7.3.1.2 缓存不命中|4.7.3.1.2 缓存不命中]]
					- [[#4.7.3.1.3 缓存不命中的种类|4.7.3.1.3 缓存不命中的种类]]
					- [[#4.7.3.1.4 缓存管理|4.7.3.1.4 缓存管理]]
				- [[#4.7.3.2 存储器层次结构概念小结|4.7.3.2 存储器层次结构概念小结]]
			- [[#4.7.4 高速缓存存储器|4.7.4 高速缓存存储器]]
				- [[#4.7.4.1 通用的高速缓存器组织结构|4.7.4.1 通用的高速缓存器组织结构]]
				- [[#4.7.4.2 直接映射高速缓存|4.7.4.2 直接映射高速缓存]]
					- [[#4.7.4.2.1 直接映射高速缓存中的组选择|4.7.4.2.1 直接映射高速缓存中的组选择]]
					- [[#4.7.4.2.2 直接映射高速缓存中的行匹配|4.7.4.2.2 直接映射高速缓存中的行匹配]]
					- [[#4.7.4.2.3 直接映射高速缓存中的字选择|4.7.4.2.3 直接映射高速缓存中的字选择]]
					- [[#4.7.4.2.4 直接映射高速缓存中不命中时的行替换|4.7.4.2.4 直接映射高速缓存中不命中时的行替换]]
					- [[#4.7.4.2.5 综合 运行中的直接映射高速缓存|4.7.4.2.5 综合 运行中的直接映射高速缓存]]
					- [[#4.7.4.2.6 为什么使用中间的位来做索引|4.7.4.2.6 为什么使用中间的位来做索引]]
				- [[#4.7.4.3 组相联高速缓存|4.7.4.3 组相联高速缓存]]
					- [[#4.7.4.3.1 组相联高速缓存中的组选择|4.7.4.3.1 组相联高速缓存中的组选择]]
					- [[#4.7.4.3.2 组相联高速缓存中的行匹配和字选择|4.7.4.3.2 组相联高速缓存中的行匹配和字选择]]
				- [[#4.7.4.4 全相联高速缓存|4.7.4.4 全相联高速缓存]]
					- [[#4.7.4.4.1 全相联高速缓存中的组选择|4.7.4.4.1 全相联高速缓存中的组选择]]
					- [[#4.7.4.4.2 全相联高速缓存中的行匹配和字选择|4.7.4.4.2 全相联高速缓存中的行匹配和字选择]]
				- [[#4.7.4.5 有关写的问题|4.7.4.5 有关写的问题]]
				- [[#4.7.4.6 一个真实的高速缓存层次结构|4.7.4.6 一个真实的高速缓存层次结构]]
				- [[#4.7.4.7 高速缓存参数的性能影响|4.7.4.7 高速缓存参数的性能影响]]
			- [[#4.7.5 编写高速缓存友好代码|4.7.5 编写高速缓存友好代码]]
			- [[#4.7.6 综合 ：高速缓存对程序性能的影响|4.7.6 综合 ：高速缓存对程序性能的影响]]
				- [[#4.7.6.1 存储器山|4.7.6.1 存储器山]]
				- [[#4.7.6.2 重新排列循环以提高空间局部性|4.7.6.2 重新排列循环以提高空间局部性]]
				- [[#4.7.6.3 在程序中利用局部性|4.7.6.3 在程序中利用局部性]]
			- [[#4.7.7 小结|4.7.7 小结]]
	- [[#5 第七章链接|5 第七章链接]]
		- [[#5.1 编译器驱动程序|5.1 编译器驱动程序]]
		- [[#5.2 静态链接|5.2 静态链接]]
		- [[#5.3 目标文件|5.3 目标文件]]
		- [[#5.4 可重定位目标文件|5.4 可重定位目标文件]]
		- [[#5.5 符号和符号表|5.5 符号和符号表]]
		- [[#5.6 符号解析|5.6 符号解析]]
			- [[#5.6.1 链接器如何解析多重定义的全局符合|5.6.1 链接器如何解析多重定义的全局符合]]
			- [[#5.6.2 与静态库链接|5.6.2 与静态库链接]]
			- [[#5.6.3 链接器如何使用静态库|5.6.3 链接器如何使用静态库]]
		- [[#5.7 重定位|5.7 重定位]]
			- [[#5.7.1 重定位条目|5.7.1 重定位条目]]
			- [[#5.7.2 重定位符号引用|5.7.2 重定位符号引用]]
				- [[#5.7.2.1 重定位 PC 相对引用|5.7.2.1 重定位 PC 相对引用]]
				- [[#5.7.2.2 重定位绝对引用|5.7.2.2 重定位绝对引用]]
		- [[#5.8 可执行目标文件|5.8 可执行目标文件]]
		- [[#5.9 加载可执行目标文件|5.9 加载可执行目标文件]]
				- [[#5.9.1.1 加载器如何工作的|5.9.1.1 加载器如何工作的]]
		- [[#5.10 动态链接共享库|5.10 动态链接共享库]]
		- [[#5.11 从应用程序中加载和链接共享库|5.11 从应用程序中加载和链接共享库]]
		- [[#5.12 位置无关代码|5.12 位置无关代码]]
			- [[#5.12.1 PIC 数据引用|5.12.1 PIC 数据引用]]
			- [[#5.12.2 PIC 函数调用|5.12.2 PIC 函数调用]]
		- [[#5.13 库打桩机制|5.13 库打桩机制]]
			- [[#5.13.1 编译时打桩|5.13.1 编译时打桩]]
			- [[#5.13.2 链接时打桩|5.13.2 链接时打桩]]
			- [[#5.13.3 运行时打桩|5.13.3 运行时打桩]]
		- [[#5.14 处理目标文件的工具|5.14 处理目标文件的工具]]
		- [[#5.15 小结|5.15 小结]]
	- [[#6 第八章异常控制流|6 第八章异常控制流]]
		- [[#6.1 异常|6.1 异常]]
			- [[#6.1.1 硬件异常和软件异常|6.1.1 硬件异常和软件异常]]
			- [[#6.1.2 异常处理|6.1.2 异常处理]]
			- [[#6.1.3 异常的类别|6.1.3 异常的类别]]
				- [[#6.1.3.1 中断|6.1.3.1 中断]]
				- [[#6.1.3.2 陷阱和系统调用|6.1.3.2 陷阱和系统调用]]
				- [[#6.1.3.3 故障|6.1.3.3 故障]]
				- [[#6.1.3.4 终止|6.1.3.4 终止]]
			- [[#6.1.4 8.1.3Linux /x86-64 系统中的异常|6.1.4 8.1.3Linux /x86-64 系统中的异常]]
				- [[#6.1.4.1 linux /x86-64 故障和终止|6.1.4.1 linux /x86-64 故障和终止]]
				- [[#6.1.4.2 linux/86-64 系统调用|6.1.4.2 linux/86-64 系统调用]]
		- [[#6.2 进程|6.2 进程]]
			- [[#6.2.1 逻辑控制流|6.2.1 逻辑控制流]]
			- [[#6.2.2 并发流|6.2.2 并发流]]
			- [[#6.2.3 私有地址空间|6.2.3 私有地址空间]]
			- [[#6.2.4 用户模式和内核模式|6.2.4 用户模式和内核模式]]
			- [[#6.2.5 上下文切换|6.2.5 上下文切换]]
		- [[#6.3 系统调用错误处理|6.3 系统调用错误处理]]
		- [[#6.4 进程控制|6.4 进程控制]]
			- [[#6.4.1 获取进程 ID|6.4.1 获取进程 ID]]
			- [[#6.4.2 创建和终止进程|6.4.2 创建和终止进程]]
			- [[#6.4.3 回收子进程|6.4.3 回收子进程]]
				- [[#6.4.3.1 判定等待集合的成员|6.4.3.1 判定等待集合的成员]]
				- [[#6.4.3.2 修改默认行为|6.4.3.2 修改默认行为]]
				- [[#6.4.3.3 检查已回收子进程的退出状态|6.4.3.3 检查已回收子进程的退出状态]]
				- [[#6.4.3.4 错误条件|6.4.3.4 错误条件]]
				- [[#6.4.3.5 wait 函数|6.4.3.5 wait 函数]]
				- [[#6.4.3.6 使用 waitpid 的示例|6.4.3.6 使用 waitpid 的示例]]
			- [[#6.4.4 让进程休眠|6.4.4 让进程休眠]]
			- [[#6.4.5 加载并运行程序|6.4.5 加载并运行程序]]
			- [[#6.4.6 利用 fork 和 execve 运行程序|6.4.6 利用 fork 和 execve 运行程序]]
		- [[#6.5 信号|6.5 信号]]
			- [[#6.5.1 信号术语|6.5.1 信号术语]]
			- [[#6.5.2 发送信号|6.5.2 发送信号]]
				- [[#6.5.2.1 进程组|6.5.2.1 进程组]]
				- [[#6.5.2.2 用 / bin/kill 程序发送信号|6.5.2.2 用 / bin/kill 程序发送信号]]
				- [[#6.5.2.3 从键盘发送信号|6.5.2.3 从键盘发送信号]]
				- [[#6.5.2.4 用 kill 函数发送信号|6.5.2.4 用 kill 函数发送信号]]
				- [[#6.5.2.5 使用 alarm 函数发送信号|6.5.2.5 使用 alarm 函数发送信号]]
			- [[#6.5.3 接收信号|6.5.3 接收信号]]
			- [[#6.5.4 阻塞和解除阻塞信号|6.5.4 阻塞和解除阻塞信号]]
			- [[#6.5.5 编写信号处理程序|6.5.5 编写信号处理程序]]
				- [[#6.5.5.1 安全的信号处理|6.5.5.1 安全的信号处理]]
				- [[#6.5.5.2 正确的信号处理|6.5.5.2 正确的信号处理]]
				- [[#6.5.5.3 可移植的信号处理|6.5.5.3 可移植的信号处理]]
			- [[#6.5.6 同步流以避免讨厌的并发错误|6.5.6 同步流以避免讨厌的并发错误]]
			- [[#6.5.7 显式的等待信号|6.5.7 显式的等待信号]]
		- [[#6.6 非本地跳转|6.6 非本地跳转]]
		- [[#6.7 操作进程的工具|6.7 操作进程的工具]]
		- [[#6.8 小结|6.8 小结]]
	- [[#7 第九章 虚拟内存|7 第九章 虚拟内存]]
		- [[#7.1 物理和虚拟寻址|7.1 物理和虚拟寻址]]
		- [[#7.2 地址空间|7.2 地址空间]]
		- [[#7.3 **9.3 虚拟内存作为缓存的工具**|7.3 **9.3 虚拟内存作为缓存的工具**]]
			- [[#7.3.1 DRAM 缓存的组织结构|7.3.1 DRAM 缓存的组织结构]]
			- [[#7.3.2 页表|7.3.2 页表]]
			- [[#7.3.3 页命中|7.3.3 页命中]]
			- [[#7.3.4 缺页|7.3.4 缺页]]
			- [[#7.3.5 分配页面|7.3.5 分配页面]]
			- [[#7.3.6 又是局部性救了我们|7.3.6 又是局部性救了我们]]
		- [[#7.4 虚拟内存作为内存管理工具|7.4 虚拟内存作为内存管理工具]]
		- [[#7.5 虚拟内存作为内存保护的工具|7.5 虚拟内存作为内存保护的工具]]
		- [[#7.6 地址翻译|7.6 地址翻译]]
			- [[#7.6.1 结合高速缓存和虚拟内存|7.6.1 结合高速缓存和虚拟内存]]
			- [[#7.6.2 利用 TLB 加速地址翻译|7.6.2 利用 TLB 加速地址翻译]]
			- [[#7.6.3 多级页表|7.6.3 多级页表]]
			- [[#7.6.4 综合：端到端的地址翻译|7.6.4 综合：端到端的地址翻译]]
		- [[#7.7 **9.7 案例研究：Intel Core i7 / Linux 内存系统**|7.7 **9.7 案例研究：Intel Core i7 / Linux 内存系统**]]
			- [[#7.7.1 Core i7 地址翻译|7.7.1 Core i7 地址翻译]]
			- [[#7.7.2 Linux 虚拟内存系统|7.7.2 Linux 虚拟内存系统]]
			- [[#7.7.3 Linux 虚拟内存区域|7.7.3 Linux 虚拟内存区域]]
			- [[#7.7.4 Linux 缺页异常处理|7.7.4 Linux 缺页异常处理]]
		- [[#7.8 内存映射|7.8 内存映射]]
			- [[#7.8.1 再看共享对象|7.8.1 再看共享对象]]
			- [[#7.8.2 再看 fork 函数|7.8.2 再看 fork 函数]]
			- [[#7.8.3 再看 execve 函数|7.8.3 再看 execve 函数]]
		- [[#7.9 动态内存分配|7.9 动态内存分配]]
			- [[#7.9.1 malloc 和 free 函数|7.9.1 malloc 和 free 函数]]
			- [[#7.9.2 为什么要使用动态内存分配|7.9.2 为什么要使用动态内存分配]]
			- [[#7.9.3 分配器的要求和目标|7.9.3 分配器的要求和目标]]
			- [[#7.9.4 碎片|7.9.4 碎片]]
			- [[#7.9.5 实现问题|7.9.5 实现问题]]
			- [[#7.9.6 隐式空闲链表|7.9.6 隐式空闲链表]]
			- [[#7.9.7 放置已分配的块|7.9.7 放置已分配的块]]
			- [[#7.9.8 分割空闲块|7.9.8 分割空闲块]]
			- [[#7.9.9 获取额外的堆内存|7.9.9 获取额外的堆内存]]
			- [[#7.9.10 合并空闲块|7.9.10 合并空闲块]]
			- [[#7.9.11 带边界标记的合并|7.9.11 带边界标记的合并]]
			- [[#7.9.12 综合：实现一个简单的分配器|7.9.12 综合：实现一个简单的分配器]]
			- [[#7.9.13 显式空闲链表|7.9.13 显式空闲链表]]
			- [[#7.9.14 分离的空闲链表|7.9.14 分离的空闲链表]]
				- [[#7.9.14.1 简单分离存储|7.9.14.1 简单分离存储]]
				- [[#7.9.14.2 \2. 分离适配|7.9.14.2 \2. 分离适配]]
				- [[#7.9.14.3 伙伴系统|7.9.14.3 伙伴系统]]
		- [[#7.10 垃圾收集|7.10 垃圾收集]]
			- [[#7.10.1 垃圾收集器的基本知识|7.10.1 垃圾收集器的基本知识]]
			- [[#7.10.2 Mark&Sweep 垃圾收集器|7.10.2 Mark&Sweep 垃圾收集器]]
		- [[#7.11 **9.11 C 程序中常见的与内存有关的错误**|7.11 **9.11 C 程序中常见的与内存有关的错误**]]
			- [[#7.11.1 间接引用坏指针|7.11.1 间接引用坏指针]]
			- [[#7.11.2 读未初始化的内存|7.11.2 读未初始化的内存]]
			- [[#7.11.3 允许栈缓冲区溢出|7.11.3 允许栈缓冲区溢出]]
			- [[#7.11.4 假设指针和它们指向的对象是相同大小的|7.11.4 假设指针和它们指向的对象是相同大小的]]
			- [[#7.11.5 造成错位错误|7.11.5 造成错位错误]]
			- [[#7.11.6 引用指针，而不是它所指向的对象|7.11.6 引用指针，而不是它所指向的对象]]
			- [[#7.11.7 误解指针运算|7.11.7 误解指针运算]]
			- [[#7.11.8 引用不存在的变量|7.11.8 引用不存在的变量]]
			- [[#7.11.9 引用空闲堆块中的数据|7.11.9 引用空闲堆块中的数据]]
			- [[#7.11.10 引起内存泄漏|7.11.10 引起内存泄漏]]
		- [[#7.12 **9.12 小结**|7.12 **9.12 小结**]]
	- [[#8 Labs|8 Labs]]
	- [[#9 Data labs|9 Data labs]]
		- [[#9.1 实验附件|9.1 实验附件]]
		- [[#9.2 实验简介|9.2 实验简介]]
			- [[#9.2.1 tips|9.2.1 tips]]
			- [[#9.2.2 第一题|9.2.2 第一题]]
			- [[#9.2.3 第二题|9.2.3 第二题]]
			- [[#9.2.4 第三题|9.2.4 第三题]]
			- [[#9.2.5 第四题|9.2.5 第四题]]
			...
			- [[#9.2.8 第七题|9.2.8 第七题]]
			- [[#9.2.9 第八题|9.2.9 第八题]]
			- [[#9.2.10 第九题|9.2.10 第九题]]
	- [[#10 Bomb Labs|10 Bomb Labs]]
		- [[#10.1 phase1|10.1 phase1]]
		- [[#10.2 phase2|10.2 phase2]]
			- [[#10.2.1 tips|10.2.1 tips]]
		- [[#10.3 phase3|10.3 phase3]]
			- [[#10.3.1 tips|10.3.1 tips]]
		- [[#10.4 phase4|10.4 phase4]]
			- [[#10.4.1 tips|10.4.1 tips]]
		- [[#10.5 phase_5|10.5 phase_5]]
		- [[#10.6 phase_6|10.6 phase_6]]
		- [[#10.7 secret_phase|10.7 secret_phase]]
			- [[#10.7.1 tips|10.7.1 tips]]
			- [[#10.7.2 简介|10.7.2 简介]]
	- [[#11 Attack Lab|11 Attack Lab]]
		- [[#11.1 前置|11.1 前置]]
		- [[#11.2 第一部分：代码注入攻击|11.2 第一部分：代码注入攻击]]
			- [[#11.2.1 level1|11.2.1 level1]]
			- [[#11.2.2 level2|11.2.2 level2]]
			- [[#11.2.3 tips|11.2.3 tips]]
			- [[#11.2.4 level3|11.2.4 level3]]
		- [[#11.3 第二部分：返回导向编程 ROP|11.3 第二部分：返回导向编程 ROP]]
			- [[#11.3.1 前言|11.3.1 前言]]
			- [[#11.3.2 攻击方式|11.3.2 攻击方式]]
				- [[#11.3.2.1 level2|11.3.2.1 level2]]
			- [[#11.3.3 gadget 表|11.3.3 gadget 表]]
				- [[#11.3.3.1 level2 - 续|11.3.3.1 level2 - 续]]
				- [[#11.3.3.2 level3|11.3.3.2 level3]]
	- [[#12 shell lab|12 shell lab]]
		- [[#12.1 linux 信号简介|12.1 linux 信号简介]]
		- [[#12.2 Shell 简介|12.2 Shell 简介]]
			- [[#12.2.1 提示|12.2.1 提示]]
		- [[#12.3 实验开始|12.3 实验开始]]
		- [[#12.4 eval|12.4 eval]]
		- [[#12.5 builtin_cmd|12.5 builtin_cmd]]
		- [[#12.6 do_bgfg|12.6 do_bgfg]]
		- [[#12.7 waitfg 函数|12.7 waitfg 函数]]
		- [[#12.8 sigchld_handler|12.8 sigchld_handler]]
		- [[#12.9 sigint_handler|12.9 sigint_handler]]
		- [[#12.10 sigtsp_handler|12.10 sigtsp_handler]]
		- [[#12.11 结果|12.11 结果]]
			- [[#12.11.1 parseline|12.11.1 parseline]]
		- [[#12.12 Cache lab|12.12 Cache lab]]
		- [[#12.13 实验附件|12.13 实验附件]]
			- [[#12.13.1 readme|12.13.1 readme]]
		- [[#12.14 概述|12.14 概述]]
		- [[#12.15 Part A：Writing a Cache Simulator|12.15 Part A：Writing a Cache Simulator]]
			- [[#12.15.1 getopt()|12.15.1 getopt()]]
			- [[#12.15.2 fscanf()|12.15.2 fscanf()]]
			- [[#12.15.3 Malloc/Free|12.15.3 Malloc/Free]]
		- [[#12.16 Part B: Optimizing Matrix Transpose|12.16 Part B: Optimizing Matrix Transpose]]
			- [[#12.16.1 PRE|12.16.1 PRE]]
			- [[#12.16.2 为什么需要缓存|12.16.2 为什么需要缓存]]
			- [[#12.16.3 缓存为什么起作用|12.16.3 缓存为什么起作用]]
			- [[#12.16.4 矩阵乘法分析|12.16.4 矩阵乘法分析]]
				- [[#12.16.4.1 先证明分块矩阵的乘法结果和不分块一致|12.16.4.1 先证明分块矩阵的乘法结果和不分块一致]]
				- [[#12.16.4.2 **3.2 那么分块矩阵为什么有用那**|12.16.4.2 **3.2 那么分块矩阵为什么有用那**]]
		- [[#12.17 开始|12.17 开始]]
		- [[#12.18 软件安装|12.18 软件安装]]
		- [[#12.19 Malloc Lab|12.19 Malloc Lab]]
			- [[#12.19.1 PRE|12.19.1 PRE]]
				- [[#12.19.1.1 背景知识|12.19.1.1 背景知识]]
				- [[#12.19.1.2 相关函数|12.19.1.2 相关函数]]
				- [[#12.19.1.3 内部碎片和外部碎片|12.19.1.3 内部碎片和外部碎片]]
				- [[#12.19.1.4 吞吐量和利用率|12.19.1.4 吞吐量和利用率]]
				- [[#12.19.1.5 查找空闲块|12.19.1.5 查找空闲块]]
			- [[#12.19.2 第一版隐式空闲链表|12.19.2 第一版隐式空闲链表]]
				- [[#12.19.2.1 合并的四种情况|12.19.2.1 合并的四种情况]]
				- [[#12.19.2.2 总结|12.19.2.2 总结]]
				- [[#12.19.2.3 开始|12.19.2.3 开始]]
				- [[#12.19.2.4 分析代码|12.19.2.4 分析代码]]
## 2 深入理解计算机操作系统 - 第一章
...
##### 4.7.4.6 一个真实的高速缓存层次结构

##### 4.7.4.7 高速缓存参数的性能影响

#### 4.7.5 编写高速缓存友好代码

#### 4.7.6 综合 ：高速缓存对程序性能的影响
...
#### 6.1.4 8.1.3Linux /x86-64 系统中的异常
...
### 7.7 **9.7 案例研究：Intel Core i7 / Linux 内存系统**
...
### 12.15 Part A：Writing a Cache Simulator
...
### 12.16 Part B: Optimizing Matrix Transpose
...
*   内部碎片 （Internal Fragmentation)  
    _Space within an allocated block that cannot be used for storing data, because it is required for some of the manager’s data structures (e.g., headers, footers, and free-list pointers)._
...
*   外部碎片 (External Fragmentation)  
    _Unused space between allocated blocks or at the end of the heap._  
...
*   吞吐量
    ...
    *   _Number of completed requests per unit time_ 单位时间内完成的请求数 (kilo-operations per second or KOPS)
    *   吞吐量衡量的是分配器的运行速度，换言之也就是在整个堆中查找空闲内存块的速度。查找空闲块的方法。
*   利用率
    
    utilization
    
    *   _The peak ratio between the aggregate amount of memory used by the driver (i.e., allocated via malloc but not yet freed via free) and the size of the heap used by your allocator._
